...
**Факт:** работа велась в `claude/discovery-documentation-efk2bo` — ветке, назначенной постоянной инструкцией сессии, которая запрещает пуш в другую ветку без явного разрешения.

`needs_verification` — владельцу нужно подтвердить, переименовать ли ветку/открыть ли PR из неё. Переименование тривиально и не влияет на содержимое.

## 3.2. Раздел K «OUT OF SCOPE» против уже реализованного
...
`needs_verification` — если владелец всё же хочет откатить PR-02/03/04 до принятия PR-01, это отдельное явное решение; технически это `git revert` трёх коммитов.

## 3.3. «Do not collect email in this PR»

**Факт:** динамический маршрут `/report/[token]` (из PR-03) содержит форму разблокировки с полем email.

**Разрешение:** маршрут закрыт тем же выключенным по умолчанию флагом и в текущем состоянии отдаёт 404. Deliverable PR-01 — `/report/sample` — **не содержит** ни поля email, ни сетевых запросов; это закреплено тестом. Даже в PR-03 email валидировался и отбрасывался, никогда не сохранялся.
...
|---|---|---|
| Имя рабочей ветки (`feat/...` vs назначенная) | `needs_verification` | Открытия PR |
| Откатывать ли уже реализованный PR-02/03/04 scope | `needs_verification` | Принятия PR-01 |
